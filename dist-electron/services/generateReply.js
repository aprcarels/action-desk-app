...
exports.getReplyUnavailableReason = getReplyUnavailableReason;
...
function generateReply(analysis, order, options) {
    ...
    if (unavailableReason) {
        return options?.allowDeterministicFallback &&
            shouldUseDeterministicFallbackReply(analysis, options.context, unavailableReason, order)
...
function getReplyUnavailableReason(analysis) {
...
function shouldUseDeterministicFallbackReply(analysis, context, unavailableReason, order) {
    if (unavailableReason === "internal_alert" ||
        unavailableReason === "non_customer_work" ||
        unavailableReason === "reply_not_needed") {
        return false;
    }
    if (unavailableReason === "not_action_required" &&
    ...
    return Boolean(order ||
        ...
        (0, caseIdentifiers_1.getIdentifierReferenceLabel)(analysis) ||
...
function needsMissingInfoReply(analysis, order) {
    const hasIdentifiers = (0, caseIdentifiers_1.hasCaseIdentifiers)(analysis);
...
function buildReply(sentences, context) {
    return [buildGreeting(context), sentences.join(" "), buildClose()].join("\n\n");
}
function buildDeterministicFallbackReply({ analysis, order, referenceLabel, generationContext, }) {
    ...
    if (order) {
        sentences.push(`Order ${order.orderNumber} is currently ${order.status}, and the shipment is ${order.shipmentStatus}${buildLastUpdatedClause(order.lastUpdated)}.`);
    }
    else if (referenceLabel) {
        sentences.push(`I have ${referenceLabel}, and I will use that to check the available case details.`);
    }
    sentences.push(buildFallbackNextStep(analysis, Boolean(order || referenceLabel)));
    return buildReply(sentences, generationContext);
}
function buildFallbackNextStep(analysis, hasReferenceContext) {
    switch (analysis.intent) {
        case "where_is_my_order":
...
function getSubjectReference(subject) {
...
function getSafeFirstName(senderName) {
    const normalizedName = normalizeInlineText(senderName);
    if (!normalizedName ||
        normalizedName.includes("@") ||
        /\b(unknown|support|team|noreply|no-reply|notification|admin)\b/i.test(normalizedName)) {
        return "";
    }
...
function buildMissingInfoReply({ analysis }) {
...
function buildOrderStatusReply({ analysis, order, referenceLabel }) {
    if (!order) {
        if (referenceLabel) {
            return buildReply([
    ...
    const issueType = (0, issueType_1.deriveIssueType)(analysis, order);
...
function buildPodReply({ order, referenceLabel }) {
    if (!order) {
        if (referenceLabel) {
            return buildReply([
...
function buildCancellationReply({ order, referenceLabel }) {
    if (!order) {
        if (referenceLabel) {
            return buildReply([
...
function buildShortShipmentReply({ order, referenceLabel }) {
    if (!order) {
        if (referenceLabel) {
            return buildReply([
...
function buildDamagedShipmentReply({ order, referenceLabel }) {
    if (!order) {
        if (referenceLabel) {
            return buildReply([
...
function buildAddressChangeReply({ order, referenceLabel }) {
    if (!order) {
        if (referenceLabel) {
            return buildReply([
...
function buildBillingReply({ order, referenceLabel }) {
    if (!order) {
        if (referenceLabel) {
            return buildReply([
...
function buildOperationalConfirmationReply({ analysis, referenceLabel }) {
...
function buildOperationalLogisticsSchedulingReply({ referenceLabel }) {
    return buildReply([
        referenceLabel
...
function buildGeneralSupportReply({ analysis, order, referenceLabel }) {
...
function buildLastUpdatedClause(value) {
