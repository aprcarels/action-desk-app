...
function generateReply(analysis, order) {
    ...
    const reviewLabel = (0, caseIdentifiers_1.getIdentifierReviewLabel)(analysis);
    ...
    if (!order) {
        ...
        if (analysis.intent === "general_support") {
            ...
            if (analysis.hasDeadlineRequest) {
                const timingLine = analysis.deadlineState === "past_due"
        ...
        if (hasIdentifiers) {
            const reviewLine = analysis.hasDeadlineRequest
        ...
        const reviewLine = issueType === "delivered_not_received"
    ...
    const acknowledgesMissingDelivery = order.shipmentStatus === "Delivered" &&
...
function getConcernLine(issueType, hasRisks) {
...
function getOpeningLine(issueType, hasRisks) {
    if (issueType === "delivered_not_received") {
...
function getScenarioMessage(order, issueType, acknowledgesMissingDelivery) {
