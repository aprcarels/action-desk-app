...
const SIGNATURE_MARKER_PATTERN = /^(thanks[,]?|thank you[,]?|best[,]?|best regards[,]?|regards[,]?|sincerely[,]?|cheers[,]?|--|sent from my (iphone|ipad|android))/i;
const DISCLAIMER_PATTERN = /(confidential|intended only for|privileged|unauthorized|do not distribute|virus|opinions expressed)/i;
function findReplyChainStart(lines) {
...
function removeTrailingDisclaimer(lines) {
...
function removeTrailingSignature(lines) {
    const startIndex = Math.max(Math.floor(lines.length * 0.5), 0);
    for (let index = startIndex; index < lines.length; index += 1) {
        const trimmedLine = lines[index].trim();
        if (!trimmedLine) {
