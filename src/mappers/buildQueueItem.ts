import type { ActionDeskAnalysisSnapshot, MailboxMessage, QueueItem } from "../domain";
import { mapSnapshotToPriorityReasons } from "./mapPriority";

export function buildQueueItem(options: {
  mailboxMessage: MailboxMessage;
  snapshot: ActionDeskAnalysisSnapshot;
  existingItem?: QueueItem | null;
}): QueueItem {
  const { mailboxMessage, snapshot, existingItem } = options;
  const now = new Date().toISOString();
  const isResolved =
    existingItem?.isResolved === true ||
  ...
  return {
    ...
    trackingNumber:
      snapshot.detectedTrackingNumber ?? mailboxMessage.extractedIdentifiers?.trackingNumber ?? null,
    workStatus,
    ownerId: existingItem?.ownerId ?? null,
    priorityScore: snapshot.priorityScore,
    priorityBand: snapshot.priorityBand,
    priorityReasons: mapSnapshotToPriorityReasons(snapshot),
