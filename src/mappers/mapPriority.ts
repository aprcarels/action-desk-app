...
import type { ActionDeskResult, PriorityBreakdownItem, RiskCode } from "../types/actionDesk";

function hasBreakdownLabel(
  breakdown: PriorityBreakdownItem[] | undefined,
  pattern: string,
): boolean {
  return breakdown?.some((item) => item.label.toLowerCase().includes(pattern)) ?? false;
}

function hasRisk(result: ActionDeskResult, risk: RiskCode): boolean {
  return result.analysis.risks.includes(risk);
}

export function mapPriorityScoreToBand(priorityScore: number): PriorityBand {
  if (priorityScore >= 85) {
    return "critical";
  }
...
export function mapActionDeskResultToPriorityReasons(
  ...
  if (
    result.analysis.hasDeadlineRequest ||
  ...
  if (
    hasRisk(result, "delay_or_no_tracking_update") ||
  ...
  if (
    hasRisk(result, "delivered_not_received") ||
  ...
  if (
    result.analysis.intent === "billing_question" ||
...
export function mapSnapshotToPriorityReasons(
  snapshot: Pick<
    ActionDeskAnalysisSnapshot,
    "priorityReasons" | "priorityScore"
  >,
): QueuePriorityReason[] {
