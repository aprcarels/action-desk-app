...
import { refreshProcessedQueue } from "./app/persistedQueueUiHelpers";
...
import { loadInboxQueue, loadRawInboxQueue } from "./services/loadInboxQueue";
import {
  ...
  async function processInboxEmailBatch(
    ...
    async function handleLoadMore() {
      ...
      try {
        if (persistedQueueEnabled) {
          const queueResult =
            await getQueueApplicationService().loadAndProcessInbox({
              cursor: nextCursor,
    ...
    const queueViewFilteredItems = pilotMode
      ...
      : filteredQueueItems;
    const visibleQueueItems = sortVisibleQueueItemsByAge(
    ...
    const topIssues = Array.from(
      ...
      .slice(0, 3);
    const selectedItem = visibleQueueItems.find(
      (item) => item.email.id === selectedEmailId,
    );
    ...
    useEffect(() => {
      ...
      const hasSelectedVisible = visibleQueueItems.some(
        (item) => item.email.id === selectedEmailId,
      );
        ...
        <div style={shellStyle}>
          ...
          {!processingStatus && totalLoadedEmails > 0 && (
            <div style={{ ...statusCardStyle, marginBottom: "20px" }}>
              ...
              {hiddenEmailCount > 0
                ? `, with ${hiddenEmailCount} hidden by the current view or filters.`
                : "."}
            </div>
          )}
