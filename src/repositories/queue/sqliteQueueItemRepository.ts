...
function deserializeQueueItem(row: QueueItemRow | undefined): QueueItem | null {
  if (!row) {
    return null;
  }

  return JSON.parse(row.record_json) as QueueItem;
}

export class SqliteQueueItemRepository implements QueueItemRepository {
  ...
  async getByMailboxMessageId(mailboxMessageId: string): Promise<QueueItem | null> {
    const row = this.database
      ...
      .get(mailboxMessageId) as QueueItemRow | undefined;
  ...
  async listActive(): Promise<QueueItem[]> {
    ...
    return rows.map((row) => JSON.parse(row.record_json) as QueueItem);
  }
  ...
  async upsertMany(items: QueueItem[]): Promise<void> {
    const transaction = this.database.transaction((nextItems: QueueItem[]) => {
